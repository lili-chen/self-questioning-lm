...
def compute_data_metrics(batch: DataProto, use_critic: bool = True, is_solver_batch: bool = False) -> dict[str, Any]:
    """
    ...
    if is_solver_batch:
        ...
        for k,v in metrics.items():
            solver_k = "solver_" + k
            solver_metrics[solver_k] = v
        metrics = solver_metrics
        metrics["solver_actor/num_valid_questions"] = (torch.sum(batch.batch["loss_mask"][:,0])).detach().item()
    return metrics
